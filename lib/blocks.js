...
/** Every placeable/scheduling-relevant block, active only, in clock order --
 *  what plan()/now()/classify() have always used. Inactive blocks are
 *  invisible here on purpose: they must not receive tasks or drive the day
 *  arc while turned off. */
function blocks() {
  return ensureSeeded()
...
/** Every block on record, active AND inactive, for a management/settings UI --
 *  the listing blocks() can never provide, since it filters ACTIVE:no out
 *  before a caller ever sees the row (U8: a block turned off had no way back
 *  on, because nothing could even show it existed to turn back on). Not used
 *  for placement/scheduling -- save() already accepted an inactive block's ID
 *  via the raw TSV read, this only fixes the "which ID?" half of the gap. */
function allBlocks() {
  return ensureSeeded()
    .map(mapRow)
    .filter(b => b.start !== null && b.end !== null)
