...
/** "Mon", "Sat", etc, for whichever date a caller is scheduling against. */
const dayAbbrev = (date = new Date()) => DAY_ABBREV[date.getDay()];
...
/** Shared row -> shaped-block mapper, used by both blocks() (active only,
 *  for placement/scheduling) and allBlocks() (every row, for management UI). */
function mapRow(r) {
...
/** Every placeable/scheduling-relevant block, active only, for the given
 *  date's weekday, in clock order -- what plan()/now()/classify() have
 *  always used. Inactive blocks are invisible here on purpose: they must
 *  not receive tasks or drive the day arc while turned off.
 *
 *  A row whose DAY column is blank matches every weekday -- backward
 *  compatible with rows written before the Sat/Sun rebuild (20 Aug 2026)
 *  gave every row its own weekday, and with ensureSeeded()'s 4-row legacy
 *  seed, which still carries no DAY column at all. */
function blocks(date = new Date()) {
  const today = dayAbbrev(date);
  return ensureSeeded()
    .filter(r => (val(r.ACTIVE) || 'yes') !== 'no')
    .filter(r => !val(r.DAY) || val(r.DAY) === today)
    .map(mapRow)
    .filter(b => b.start !== null && b.end !== null)
    .sort((a, b) => a.start - b.start);
}

/** Every block on record, every weekday, active AND inactive, for a
 *  management/settings UI (and for the Planner calendar, which groups these
 *  by their own `day` field client-side rather than asking the server for
 *  one weekday at a time) -- the listing blocks() can never provide, since
 *  it filters both ACTIVE:no and other weekdays out before a caller ever
 *  sees the row (U8: a block turned off had no way back on, because nothing
 *  could even show it existed to turn back on). Not used for placement/
 *  scheduling -- save() already accepted an inactive block's ID via the raw
 *  TSV read, this only fixes the "which ID?" half of the gap. */
function allBlocks() {
  return ensureSeeded()
    .map(mapRow)
    .filter(b => b.start !== null && b.end !== null)
 ...
 */
// BT26082416 (partial -- the soft-cap/confirmation half is bounced to
// PT26082703, plan.md, since it has no existing UI action to attach to;
// this half stands on its own regardless of how that's resolved): once a
// block's task count is set, divide its available time equally across
// them with up to 2 mini-breaks inserted. 5 minutes/break is a build-time
// default, not specified by the row -- easy to change in one place.
const SUBBLOCK_BREAK_MINUTES = 5;
const SUBBLOCK_MAX_BREAKS = 2;

/** Returns one {offsetMinutes, durationMinutes} per task index, offsets
 *  relative to the block's own start. Breaks are spread evenly across the
 *  gaps between tasks (never more breaks than gaps). A single task gets
 *  the whole block, no breaks -- nothing to space out. */
function divideBlockTime(totalMinutes, taskCount) {
  if (taskCount <= 0) return [];
  if (taskCount === 1) return [{ offsetMinutes: 0, durationMinutes: totalMinutes }];
  const breaks = Math.min(SUBBLOCK_MAX_BREAKS, taskCount - 1);
  const usable = Math.max(0, totalMinutes - breaks * SUBBLOCK_BREAK_MINUTES);
  const perTask = usable / taskCount;
  const breakAfterIndex = new Set();
  for (let b = 1; b <= breaks; b++) {
    const gapIdx = Math.round((b * (taskCount - 1)) / (breaks + 1)) - 1;
    breakAfterIndex.add(Math.max(0, Math.min(taskCount - 2, gapIdx)));
  }
  const slots = [];
  let offset = 0;
  for (let i = 0; i < taskCount; i++) {
    slots.push({ offsetMinutes: offset, durationMinutes: perTask });
    offset += perTask;
    if (breakAfterIndex.has(i)) offset += SUBBLOCK_BREAK_MINUTES;
  }
  return slots;
}

/** offsetMinutes (from a block's start) -> "HH:MM", wrapping past midnight
 *  the same way a wrapping block (Rest) already does. */
function clockAt(startMinutes, offsetMinutes) {
  const total = Math.round(startMinutes + offsetMinutes) % 1440;
...
function plan({ tasks = [], people = [], date = new Date() } = {}) {
  ...
  return {
    date: today, now: n,
    blocks: bs.map(b => {
      const bucket = placed.get(b.id) || [];
      const subSlots = divideBlockTime(b.minutes, bucket.length);
      return {
        ...b,
        tasks: bucket.map((t, i) => ({
          ...t,
          subBlock: subSlots[i] ? {
            durationMinutes: Math.round(subSlots[i].durationMinutes),
            startClock: clockAt(b.start, subSlots[i].offsetMinutes),
            endClock: clockAt(b.start, subSlots[i].offsetMinutes + subSlots[i].durationMinutes),
          } : null,
        })),
        full: bucket.length >= b.slots,
...
module.exports = { init, blocks, allBlocks, now, classify, plan, save, TASK_MINUTES, divideBlockTime, clockAt };
