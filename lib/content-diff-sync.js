...
const CONTENT_TSVS = [
  ...
  // FL26091207: campus.tsv is genuinely hand-authored (PL26090403 exists
  // entirely because of that) -- editing it in a text editor and having the
  // edit silently never reach vault.db was a real, live workflow gap. The
  // other learning registry TSVs (progress/resume/library_selection/
  // library_meta/audio_versions, now groups/modules_meta/contributions too)
  // stay excluded deliberately -- they're machine-written in practice.
  { relPath: 'learning/campus.tsv', key: 'ID' },
];
...
const EXTRA_RAW_FILES = [
  ...
  // CL26091120: jargon-levels.tsv is hand-authored content, not a machine-
  // written registry, so it belongs on the sync path -- but as a RAW file
  // rather than a CONTENT_TSVS entry. A CONTENT_TSVS row-upsert writes
  // through store.read/append/rewrite, which needs the collection declared
  // in default-schema.js; an undeclared collection silently no-ops under
  // VAULT_STORE_ENGINE=sqlite (the FI26091501 shape). Nothing reads this
  // file as a store collection yet, so the raw path carries it with no
  // schema dependency. If PL26091203's level selector ends up wanting
  // row-level reads, declare the collection first, then move this line --
  // moving it without declaring would fail silently, which is the whole
  // reason it is written down here.
  'learning/_standards/jargon-levels.tsv',
];
...
/** True for relPaths shaped exactly like a discoverLessonFiles() entry (learning/<courseId>/<file>.md, courseId not underscore-prefixed) -- used to scope raw_blobs pruning to lesson rows only, never EXTRA_RAW_FILES or any other raw content living under learning/. */
function isLessonPath(relPath) {
  const parts = relPath.split('/');
  return parts.length === 3 && parts[0] === 'learning' && !parts[1].startsWith('_') && parts[2].endsWith('.md');
}
...
function syncTsvCollection(store, memoryDir, relPath, keyCol, state, results, { dryRun }) {
  ...
  for (const [key, fileRow] of fileByKey) {
    ...
    if (fileChanged && dbChanged) {
      ...
      // DB edited live (app/web) since last sync -- leave it alone,
...
function runContentDiffSync({ store, memoryDir, statePath, dryRun = false }) {
  ...
  // FL26091301: a lesson .md renamed or deleted on disk was never reached by
  // the loop above (it only visits keys discoverLessonFiles() still finds)
  // and its raw_blobs row survived forever -- same shape as the TSV stale-row
  // prune above, mirrored here for the raw store. Scoped to lesson-shaped
  // keys only (isLessonPath) so this never touches EXTRA_RAW_FILES or other
  // raw content that happens to live under learning/.
  const onDiskLessonSet = new Set(discoverLessonFiles(memoryDir));
  const dbLessonKeys = (typeof store.rawKeysWithPrefix === 'function' ? store.rawKeysWithPrefix('learning/') : [])
    .filter(isLessonPath);
  const staleRawKeys = dbLessonKeys.filter((k) => !onDiskLessonSet.has(k));
  for (const relPath of staleRawKeys) {
    if (!dryRun) store.rawDelete(relPath);
    results.push({ kind: 'raw-pruned-stale-db-row', relPath });
    delete state[`raw:${relPath}`];
  }
