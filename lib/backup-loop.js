...
function createBackupLoop({
  ...
  async function runOnce() {
    ...
    try {
      ...
      // FI26091604: prove the snapshot is restorable BEFORE it is published.
      //
      // The 31 Aug 2026 generation is 15MB that looks like a backup and is not
      // one -- its manifest carries no saltHex, so no key can be derived and the
      // file cannot be opened at all. It sat in the retention window for over
      // two weeks reporting `ok`, and surfaced only when something finally tried
      // to open it for an unrelated reason. A backup nobody opens is
      // indistinguishable from a backup that works.
      //
      // Two things are checked, and the first is the one that would have caught
      // that generation: an ENCRYPTED store must have produced a salt. The
      // `catch` above deliberately tolerates a missing salt file, because the
      // tsv engine legitimately has none -- but on the sqlite engine that same
      // silence means the snapshot is being shipped unopenable. Distinguish the
      // two by asking the store whether it can verify at all, rather than by
      // guessing from the engine name.
      const verdict = typeof store.verifySnapshot === 'function'
        ? store.verifySnapshot(tmp, saltHex)
        : null;

      if (verdict && !verdict.ok) {
        // Refuse to publish. Pushing an unrestorable generation is strictly
        // worse than skipping a pass: it still consumes a retention slot, so it
        // evicts a generation that did work.
        auditLog.log('vault_backup_unrestorable', { error: verdict.error, hadSalt: !!saltHex });
        result = {
          ok: false, startedAt, finishedAt: new Date().toISOString(),
          stage: 'verify', verified: false,
          error: `refused to publish an unrestorable snapshot: ${verdict.error}`,
        };
      } else {
        if (verdict) auditLog.log('vault_backup_verified', { tables: verdict.tables });
        ...
        if (!pushResult.ok) {
          ...
          result = {
            ...
            ref: pushResult.ref,
            // FI26091604: recorded so a reader of /backup/status can tell a pass
            // that was proven restorable from one that merely uploaded.
            verified: verdict ? verdict.tables : null,
