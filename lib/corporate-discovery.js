 ...
 *
 * Scans `Sconl/Core/Axial/Visionary/Corporate/` (a tree outside vault's
 * own collection-sync's known paths, hence listRemoteFoldersAbsolute --
 * lib/onedrive-list-folders.js, extracted from the now-retired
 * onedrive-sync.js since this is the one piece BI26083005 didn't retire)
 * for engagement folders
 ...
 */
async function discoverOrgs(graph, { corporateRoot = CORPORATE_ROOT, onedriveSync = require('./onedrive-list-folders') } = {}) {
  ...
  const orgs = listing.folders
    .filter((f) => !f.name.startsWith('_')) // underscore prefix = internal/archive, not a live engagement (see FC26082401)
    .map((f) => {
