  ...
  function repairColumnShiftedHeaders() {
    ...
    for (const rel of Object.keys(schema)) {
      ...
      try {
        ...
        // Corruption confirmed: header starts with a tab. Heal in place.
        keepPreviousVersion(rel, raw, 'leading-tab-corruption-before-repair');
        const cleaned = lines.map(l => l.startsWith('\t') ? l.slice(1) : l).join('\n');
