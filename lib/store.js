...
function createVaultStore({ memoryDir, logsDir, schema = defaultSchema, auditLog = { log: () => {} } }) {
  ...
  // -- write-then-push hook (SYNC1, 2026-08-18) ---------------------------------
  ...
  // in the actual push function via setPushHook. Fire-and-forget: a write
  // must never block on network, so this never awaits and only logs on
  // failure, same spirit as keepPreviousVersion's own catch-and-log.
  let pushHook = null;
  function setPushHook(fn) { pushHook = fn; }
  function firePush(relPath) {
    if (!pushHook) return;
    Promise.resolve(pushHook(relPath)).then((r) => {
  ...
  return {
    schema,
    read, append, rewrite, rawRead, rawWrite, listDir, keepPreviousVersion,
