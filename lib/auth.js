...
/** Same length rule verifyPin() checks a submitted PIN against, exposed so a
 *  set-PIN route can reject a bad new PIN before ever hashing/storing it,
 *  without duplicating (and risking drifting from) the actual constants. */
function pinFormatOk(pin) {
  const clean = String(pin || '').replace(/\D/g, '');
  return clean.length >= PIN_MIN_DIGITS && clean.length <= PIN_MAX_DIGITS && clean === String(pin || '');
}
...
function createAuthModule(opts) {
  ...
  function authenticate(req) {
    ...
    // BS26090501: dev-only, loopback-gated (enforced at server.js boot), env-only -- never request-derived.
    if (process.env.ISCONL_DEV_NO_AUTH === '1') {
      auditLog.log('dev_auth_bypass', { engine: 'vault', ip, path: req.url });
      return { ok: true, ip, via: 'dev_bypass' };
    }
...
module.exports = { createAuthModule, safeEqual, totpSeedUsable, pinDigest, pinFormatOk };
