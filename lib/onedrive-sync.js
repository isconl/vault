...
const { parseTSVText, stripBOM } = require('./tsv');
...
async function pullToLocal(graph, store, relPath, { force = false } = {}) {
  ...
  // FI26082901: reject the pull if the remote content is corrupted.
  // 1. Remote header starts with a tab.
  // 2. Or data rows have ID='-' across non-empty records (column-shift).
  if (isTSV(relPath) && remote.raw) {
    if (remote.raw.startsWith('\t')) {
      return {
    ...
    const lines = stripBOM(remote.raw).trim().split(/\r?\n/).filter(l => l.trim());
    if (lines.length > 1) {
      const h = lines[0].split('\t');
      const d = lines[1].split('\t');
      if (h[0] === 'ID' && (d[0] === '-' || d[0] === '') && d.some(v => v !== '-' && v !== '')) {
        return {
          collection: relPath, ok: false,
          error: 'FI26082901: remote rows have corrupted/shifted ID columns -- pull refused to protect local copy',
        };
      }
    }
  }
