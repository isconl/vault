...
function httpsRequestOnce(options, postData) {
  return new Promise((resolve, reject) => {
    ...
    req.on('error', reject);
    // .code = 'ETIMEDOUT' (not just the bare message) so this lands in
    // httpsRequest()'s TRANSIENT_NETWORK_ERRORS retry below -- found 18 Aug:
    // a plain `Error('timeout')` has no .code, so `.has(e.code)` was always
    // false and every socket timeout skipped the retry path entirely and
    // failed the collection outright (the recurring "learning/viva-tasks
    // (timeout)" sync failures, unlike the ECONNRESET class fixed 17 Aug).
    ...
    // FI26082001: req.write() throws synchronously (not a rejection) on a
    // non-string/Buffer postData -- graphRequest() now normalizes its own
    // body before this point, but this catch is the last line of defense
    // for any other caller of httpsRequest/httpsRequestOnce so a bad type
    // fails this one call instead of taking down the process.
    try {
...
// collection catch and surfaced as a hard sync failure (sync-loop.js
// itself is retired -- BI26083005 -- but the retry logic below still
// protects every other caller of httpsRequest/httpsRequestOnce the same
// way). These codes are
...
// 4 tries, not 3 (18 Aug) -- 3 wasn't enough headroom for the timeout class
// specifically: found live, course-folder pulls (learning/viva-model and
// siblings) still failing whole sync passes after the ETIMEDOUT fix above,
// because listRemoteFolder's one /children request is on the critical path
// for the entire folder and a slow-but-not-dead connection can outlast two
// quick retries (400ms+800ms) under real network jitter.
async function httpsRequest(options, postData, attempt = 1) {
  try {
    return await httpsRequestOnce(options, postData);
  } catch (e) {
    if (attempt < 4 && TRANSIENT_NETWORK_ERRORS.has(e.code)) {
...
function extractGraphError(res) {
  return typeof res.data === 'object' ? JSON.stringify(res.data).slice(0, 200) : String(res.data || '').slice(0, 200);
}

/**
 * Binary-safe raw GET, collecting chunks as Buffers (Buffer.concat), never
 * `data += chunk` -- that string-concatenation pattern (what
 * httpsRequestOnce above and graphRequest's own redirect-follow use) decodes
 * every chunk as UTF-8 as it arrives, which is correct for JSON/text
 * responses but silently corrupts arbitrary binary content (any byte
 * sequence that isn't valid UTF-8 gets mangled into replacement
 * characters). Not a problem for anything this module downloaded before
 * (JSON/TSV/markdown text) -- it IS a problem for BI26083004's backup
 * fetch(), which downloads the encrypted `.db` file itself: a byte-mangled
 * download would silently fail to decrypt (or worse, decrypt into garbage)
 * with no error at download time. Found while building that row, not a
 * pre-existing bug report -- fixed here rather than reusing the
 * text-decoding path for a case it was never correct for.
 */
function binaryGetOnce(options) {
  return new Promise((resolve, reject) => {
    const req = https.request(options, (res) => {
      const chunks = [];
...
/**
 * Download a drive item's raw content (already-resolved Graph drive path,
 * e.g. from resolveRemotePath) as a Buffer, following the same
 * :/content -> pre-signed-redirect pattern graphRequest uses for text, but
 * never decoding bytes as a string. The redirect target is a different
 * host (blob storage CDN) already authenticated by its own signed query
 * string, so -- same reasoning as graphRequest's redirect handling -- the
 * follow-up request deliberately does NOT carry the Graph bearer token.
 *
 * @param {object} graph - a createGraphClient() instance
 * @param {string} drivePath - already resolveRemotePath()-prefixed
 * @param {Function} [binaryGetFn] - injectable for testing without a live network call
 */
async function downloadBinary(graph, drivePath, binaryGetFn = binaryGetOnce) {
  const token = await graph.getValidToken();
  if (!token) return { ok: false, error: 'Microsoft 365 not connected.' };
  const reqPath = `/v1.0/me/drive/root:/${encodeURIComponent(drivePath).replace(/%2F/g, '/')}:/content`;
  let res = await binaryGetFn({ hostname: 'graph.microsoft.com', path: reqPath, method: 'GET', headers: { Authorization: `Bearer ${token}` } });
  if ([301, 302, 303, 307, 308].includes(res.status) && res.headers?.location) {
    const u = new URL(res.headers.location);
    res = await binaryGetFn({ hostname: u.hostname, path: u.pathname + u.search, method: 'GET', headers: {} });
  }
  if (res.status !== 200) return { ok: false, status: res.status, error: `download failed (status ${res.status})` };
  return { ok: true, buffer: res.buffer };
}

// Chunk size for a resumable upload session: a multiple of 320 KiB (Graph's
// required alignment for every chunk but the last).
const DEFAULT_UPLOAD_CHUNK_SIZE = 327680 * 12; // ~3.93MB

/**
 * FI26082802: the chunked-PUT loop for an already-created resumable upload
 * session, shared by onedrive-browse.js's uploadLarge() and
 * BI26083004's onedrive-target.js -- so the truncation-detection fix below
 * applies to both instead of needing to be ported twice.
 *
 * The loop itself is unchanged from what uploadLarge() already did; the
 * fix is the POST-loop verification: a truncated transfer was observed to
 * exit the loop with every per-chunk status check passing (200/201/202)
 * and still return {ok:true} on a file that landed with fewer bytes than
 * sent. After the loop, re-fetch the uploaded item's own metadata (not
 * just trust the last chunk response) and compare its reported `size`
 * against the buffer actually sent; on any disagreement, delete the
 * partial remote object (never leave a corrupted file looking like a
 * successful upload) and report the real byte counts instead of {ok:true}.
 *
 * @param {object} graph - a createGraphClient() instance (for the post-upload verify GET + DELETE)
 * @param {string} uploadUrl - Graph's `uploadUrl` from a createUploadSession response
 * @param {Buffer} buffer
 * @param {number} [chunkSize]
 * @param {Function} [httpsRequestFn] - injectable for testing without a live network call, same pattern as createGraphClient's own opts.httpsRequestFn
 */
async function uploadVerified(graph, uploadUrl, buffer, chunkSize = DEFAULT_UPLOAD_CHUNK_SIZE, httpsRequestFn = httpsRequest) {
  ...
  while (start < total) {
    ...
    if (![200, 201, 202].includes(last.status)) return { ok: false, error: extractGraphError(last) };
    start = end + 1;
  }

  const uploadedItem = last && last.data;
  const id = uploadedItem && uploadedItem.id;

  // Re-fetch metadata rather than trusting the last chunk response's own
  // reported size -- the whole point of this guard is that a truncated
  // transfer was observed to report a plausible-looking success.
  let verified = uploadedItem;
  if (id) {
    const metaRes = await graph.graphRequest(`/v1.0/me/drive/items/${encodeURIComponent(id)}`);
    if (metaRes.status === 200) verified = metaRes.data;
  }

  if (!verified || verified.size !== total) {
    if (id) await graph.graphRequest(`/v1.0/me/drive/items/${encodeURIComponent(id)}`, { method: 'DELETE' });
    return { ok: false, error: `upload truncated: uploaded ${verified ? verified.size : 'unknown'} bytes, expected ${total}` };
  }
...
function createGraphClient(opts) {
  ...
  async function graphRequest(pathAndQuery, { method = 'GET', body = null, headers = {} } = {}) {
    ...
    // FI26082001: a caller passing a plain object (instead of a
    // pre-serialized string/Buffer) used to reach req.write() raw, where
    // Node throws a synchronous TypeError that escaped as an uncaught
    // exception and killed the whole process -- every vault-backed service
    // (pulse/scope/circle/spark/hub) went down with it. Normalize here so a
    // caller mistake degrades to "this one call gets serialized correctly"
    // instead of "the process dies."
    const sendBody = (body != null && typeof body !== 'string' && !Buffer.isBuffer(body))
      ? JSON.stringify(body)
      : body;
...
module.exports = { createGraphClient, msGraphTokenExpired, httpsRequest, uploadVerified, downloadBinary };
