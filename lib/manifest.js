...
module.exports = {
  ...
  capabilities: [
    ...
    { name: 'ventures.discovery.run', method: 'POST', path: '/ventures/discovery/run', description: 'BN26090610: re-run the OneDrive (_ace) venture discovery scan immediately and push any new candidates into pulse\'s ventures registry, without waiting for the interval timer.' },
    ...
    { name: 'library.catalog.list', method: 'GET', path: '/library/catalog', description: 'BL26082601: the iSpark Library\'s published track/course catalog, read directly from LIBRARY_MEMORY_DIR (unconfigured on the main fleet -- only a tenant instance sets this).' },
    { name: 'library.selection.get', method: 'GET', path: '/library/selection', description: 'This tenant\'s currently-enabled Library course IDs.' },
    { name: 'library.selection.set', method: 'POST', path: '/library/selection', description: 'Replace this tenant\'s Library selection (body: {courseIds: string[]}) -- pulls newly-selected course content in, disables (never deletes) deselected courses.' },
    ...
    { name: 'contentSync.run', method: 'POST', path: '/content-sync/run', description: 'Force an immediate file-to-DB content diff-sync pass (course lesson .md files, courses.tsv), independent of the interval timer.' },
    { name: 'contentSync.status', method: 'GET', path: '/content-sync/status', description: 'Whether the content-sync loop is running and the result of its last pass.' },
    ...
    { name: 'cycleTheme.get', method: 'GET', path: '/cycle-theme', description: 'The equicycle theme word for one cycle (?cycleKey=<eqYear>-<cycleNum>, defaults to the current cycle) -- {cycleKey, theme:string|null} from scope/cycle_themes.tsv, or theme:null if no override was written for that cycle. Client falls back to the default 13-name array in that case.' },
    { name: 'cycleTheme.set', method: 'POST', path: '/cycle-theme', description: 'Write (or clear) a cycle theme override -- body: {cycleKey, theme}. An empty/omitted theme removes the override, reverting that cycle to the default 13-name rotation.' },
    ...
    { name: 'study.audio.get', method: 'GET', path: '/learning/audio', description: 'Latest ready narration for one module (?course=&file=): version, voice, duration, a playable url. {ok:false} if none generated yet.' },
    { name: 'study.audio.generate', method: 'POST', path: '/learning/audio/generate', description: 'Generate (or regenerate, if the module text changed) Chatterbox narration for one module (body: {course, file}), upload it to OneDrive, and record a new version row. Skips and returns the existing version if the module text is unchanged since the last generation.' },
  ],
};
