...
function createSqliteStore({ memoryDir, logsDir, schema = defaultSchema, auditLog = { log: () => {} }, dbKeyPassphrase }) {
  ...
  function rewrite(relPath, fn, opts = {}) {
    ...
    // FI26091501: append() self-heals a missing table via ensureTableFor()
    // (see above); rewrite() used to skip that and silently no-op instead
    // (fn not even called) whenever a schema path's table hadn't been
    // created yet -- e.g. a collection added to default-schema.js since
    // the process last booted. The caller got back the same `0` a
    // successful no-op write returns, so "nothing persisted" and "nothing
    // needed persisting" were indistinguishable -- this is exactly why
    // learning/groups.tsv edits kept silently failing to reach the live
    // vault. Now matches append(): ensure the table exists before doing
    // anything else. A bootstrapped-but-empty table (0 rows) still calls
    // fn as before, e.g. to seed rows via rewrite -- only the massacre
    // guard below treats it specially (currentRows.length > 1 is false,
    // so it never fires).
    ensureTableFor(relPath);
    const table = tableNameFor(relPath);
    const cols = schemaCols(relPath);
    ...
    // FI26091501: this refusal used to also return a bare `0` -- identical
    // to "wrote successfully, nothing was removed" -- so a refused bulk
    // delete/replace was indistinguishable from a normal no-op write at
    // every layer above this one (the PUT /vault/:collection route
    // answered {ok:true, removed:0} either way). Returns `null` instead --
    // NOT a numeric sentinel like -1, because `lost` is already a signed
    // count (it goes negative when a rewrite ADDS rows, see the
    // empty-to-populated case below), so no integer value is safely
    // reserved for "refused." `null` is unambiguous and easy for a caller
    // to check; see server.js's PUT handler, the only caller that
    // surfaces this value to an HTTP response.
  ...
  function rawDelete(relPath) {
    const before = rawRead(relPath);
    if (!before) return false;
    keepPreviousVersion(relPath, before, 'rawDelete');
    db.prepare('DELETE FROM raw_blobs WHERE rel_path = ?').run(relPath);
    auditLog.log('vault_raw_deleted', { file: relPath, bytesBefore: before.length });
    firePush(relPath);
    return true;
  }

  /** All raw_blobs rel_paths currently stored under a given prefix (e.g. 'learning/'), for pruning callers that need to diff against an on-disk set. */
  function rawKeysWithPrefix(prefix) {
    return db.prepare('SELECT rel_path FROM raw_blobs WHERE rel_path LIKE ?').all(`${prefix}%`).map((r) => r.rel_path);
  }
  ...
  // BI26091201: "does this DB actually hold content?" -- the backup loop asks
  // this before pushing, so a freshly-bootstrapped or failed-restore database
  // can never overwrite good backup history (PI26091001: a fresh Windows
  // checkout pushed three ~196KB generations into the shared OneDrive history
  // alongside another machine's real ~24MB ones). Counted as ROWS across a
  // fixed set of key content collections plus the raw blob store, not file
  // size: size was only ever a proxy for the thing that actually matters, and
  // an encrypted page-aligned DB file has a nontrivial floor even when empty.
  // The collections below are the ones every real vault has content in --
  // ensureVault() creates all 47 schema tables empty on boot, so table
  // EXISTENCE proves nothing; only rows do.
  const CONTENT_COLLECTIONS = [
    'learning/courses.tsv',
    'scope/tasks.tsv',
    'circle/people.tsv',
    'spark/ideas.tsv',
  ...
  function contentStats() {
    const counts = {};
    let totalRows = 0;
    for (const relPath of CONTENT_COLLECTIONS) {
      let n = 0;
      try {
        const table = tableNameFor(relPath);
        if (tableExists(table)) n = db.prepare(`SELECT COUNT(*) AS c FROM "${table}"`).get().c;
      } catch { n = 0; }
      counts[relPath] = n;
      totalRows += n;
    }
    // Raw blobs (lesson markdown and friends) are the bulk of a real vault's
    // bytes, and the first thing a half-restored DB is missing.
    let rawBlobs = 0;
    try { rawBlobs = db.prepare('SELECT COUNT(*) AS c FROM raw_blobs').get().c; } catch { rawBlobs = 0; }
    counts.raw_blobs = rawBlobs;
    totalRows += rawBlobs;
  ...
  /**
   * FI26091604: can this snapshot actually be restored with the salt we are
   * about to ship alongside it?
   *
   * Why a backup pass cannot answer that for itself: the encryption key is
   * scrypt(passphrase, salt), the passphrase lives only in here, and the salt
   * travels separately in the manifest. A pass can therefore write a perfectly
   * valid encrypted file, ship a manifest that cannot unlock it, and report
   * success -- which is exactly what happened to the 31 Aug 2026 generation,
   * found unopenable only when something finally tried to open it weeks later.
   *
   * So this does the restore, not a proxy for it: derive the key from the salt
   * the caller is really going to publish, open the snapshot read-only with it,
   * and read `sqlite_master`. If any of that fails the file is not a backup,
   * whatever its size says.
   *
   * @param {string} filePath - the snapshot to verify
   * @param {string} saltHex - the salt the manifest will carry, hex-encoded
   * @returns {{ok: boolean, tables?: number, error?: string}}
   */
  function verifySnapshot(filePath, saltHex) {
    if (!saltHex) return { ok: false, error: 'no salt supplied -- the snapshot could never be decrypted' };
    let probe;
    try {
      const probeKey = crypto
        .scryptSync(dbKeyPassphrase, Buffer.from(saltHex, 'hex'), 32, SCRYPT_PARAMS)
        .toString('hex');
      probe = new Database(filePath, { readonly: true });
      probe.exec(`PRAGMA key = "x'${probeKey}'"`);
      // Reading sqlite_master is the cheapest operation that genuinely needs the
      // page cipher to be right. A wrong key fails here with SQLITE_NOTADB
      // rather than returning something plausible.
      const tables = probe.prepare("SELECT count(*) c FROM sqlite_master WHERE type='table'").get().c;
      if (!tables) return { ok: false, error: 'snapshot opened but contains no tables' };
      return { ok: true, tables };
    } catch (e) {
      // Never leak the passphrase or the derived key through an error string.
      return { ok: false, error: String(e.code || e.message || e).slice(0, 120) };
    } finally {
      if (probe) { try { probe.close(); } catch { /* nothing useful to do */ } }
    }
  }

  return {
    schema, memoryDir,
    read, append, rewrite, rawRead, rawWrite, rawDelete, rawKeysWithPrefix, statMtimeMs, listDir, keepPreviousVersion,
    contentStats,
    ensureVault, ensureVaultColumns, bootRepair, snapshotToFile, verifySnapshot,
    setPushHook,
  };
}
