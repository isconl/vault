...
function accessToken() {
  // Same reasoning as envOrUser's own explicit-empty-override guard above:
  // an explicit process.env.BWS_ACCESS_TOKEN (even '', a test disabling
  // secrets) must win outright and never fall through to the token file --
  // otherwise a machine with a persisted bootstrap credential at
  // ~/.isconl/bws-access-token silently restores the real token underneath
  // a test that thought it had disabled secrets (found live 31 Aug 2026,
  // FI26083101 / vault/test/server.test.js's "GET /secrets/status" test).
  const explicitEnvOverride = Object.prototype.hasOwnProperty.call(process.env, 'BWS_ACCESS_TOKEN');
  const fromEnv = envOrUser('BWS_ACCESS_TOKEN');
  if (fromEnv || explicitEnvOverride) return fromEnv;
...
function get(name, fallback = '') {
  ...
  // leaked real Bitwarden-sourced secrets into tests that explicitly
  // tried to clear them.
