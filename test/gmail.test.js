...
test('parseFrom splits "Display Name <email>" and lowercases the address', () => {
  assert.deepEqual(parseFrom('Joel Tambaur <Joel@Example.com>'), { name: 'Joel Tambaur', email: 'joel@example.com' });
});
...
test('getMessage parses headers, from, and the plain-text body', async () => {
  const google = fakeGoogle(() => ({
    ...
    data: {
      ...
      payload: {
        headers: [
          { name: 'From', value: 'Joel Tambaur <joel@example.com>' },
  ...
  assert.equal(msg.from.email, 'joel@example.com');
...
test('sendMessage builds a base64url RFC822 message and posts it to the send endpoint', async () => {
  ...
  const r = await sendMessage(google, { to: 'joel@example.com', subject: 'Re: proposal', body: 'sounds good', threadId: 't1', inReplyTo: '<abc@mail.gmail.com>' });
  assert.equal(r.ok, true);
  ...
  assert.match(decoded, /To: joel@example\.com/);
