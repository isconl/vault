...
test('findPersonByEmail matches case-insensitively and returns null for no match or empty input', () => {
  const people = [{ ID: 'joel', EMAIL: 'Joel@Example.com' }];
  assert.equal(findPersonByEmail(people, 'joel@example.com').ID, 'joel');
...
test('syncGmailAccount writes a new inbox.tsv row per message, matched by EMAIL', async () => {
  const store = fakeStore({ 'circle/people.tsv': [{ ID: 'joel', NAME: 'Joel Tambaur', EMAIL: 'joel@example.com' }] });
  const gmail = fakeGmail({
    ids: ['m1'],
    messages: { m1: { ok: true, id: 'm1', threadId: 't1', from: { name: 'Joel Tambaur', email: 'joel@example.com' }, subject: 'Re: proposal', date: '2026-08-21', body: 'sounds good' } },
  });
  ...
  assert.equal(row.PERSON_ID, 'joel');
  assert.equal(row.DIRECTION, 'in');
  assert.equal(row.CHANNEL, 'gmail');
  assert.equal(row.SOURCE, 'gmail:default:m1');
  assert.equal(row.SENDER, 'Joel Tambaur <joel@example.com>', 'SENDER carries the real address parseable back out, not just the display name');
});
